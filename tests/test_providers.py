...
def test_search_attaches_cached_pins_and_google_summaries():
    ...
    with patch.object(p, "zip_table", return_value=TABLE), patch.object(p, "_npi_query", return_value=[row]), patch.object(p, "cache_get", return_value={}):
        r0 = p.search("doctors", "cardiology", zip_code="75075")["results"][0]
    assert r0["google"] is None
    assert r0["approx"] is True and abs(r0["lat"] - 33.02) < 0.01 and abs(r0["lon"] + 96.74) < 0.01  # instant placeholder at the ZIP centre
    ...
    assert r2["google"] is None and r2["approx"] is True  # the old practice's rating and exact pin never follow a move
