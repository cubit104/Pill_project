...
## Retraining v2 — phone-tolerant reader, base + large in one run (Google Colab, A100)

Notebook: `ml/notebooks/pillseek_ocr_v2_colab.ipynb` (generated by `make_ocr_v2_notebook.py`; edit the
generator, not the notebook). Starts from the v1 fine-tunes and trains them further on *phone-style
synthesis*: pill cut-outs placed on wood / fabric / skin / paper textures at any rotation, imprint-only
side-lighting (never the outline — v2.0 learned the outline as a "1"), glare, tilt, blur, noise, JPEG,
reader-style crops, plus ~3% background-only samples with an empty label (silence on blank sides).

Everything reusable is cached in the Supabase bucket `LLM_MODELS/training-cache/` (images zip, cut-outs
zip, per-epoch checkpoints, finished zips, REPORT_<size>.txt); the v1 zips are read from
`LLM_MODELS/pill-reader/`. A dropped session resumes at the last finished epoch and nothing is downloaded
twice. The notebook needs the service_role key once, stored in Colab Secrets as `SUPABASE_SERVICE_KEY`.

1. `venv\Scripts\python.exe ml\scripts\export_manifest.py` → `ml/scripts/manifest.json` (git-ignored)
2. Colab, A100: add the `SUPABASE_SERVICE_KEY` secret (key icon), run all; cell 3 asks for `manifest.json`
   the first time only.
3. Cell 9 trains base then large (~1 h + ~2.5 h), printing BEFORE/AFTER accuracy on held-out catalog
   and phone-style images plus blank-side silence. Deploy only if AFTER is clearly better than BEFORE.
4. Zips land in the bucket as `training-cache/pill_trocr_<size>_v2.zip`. Reader box: unzip into `~/pillseek-reader/`,
   point `PILL_TROCR_DIR` (base) / `PILL_TROCR_DIR2` (large) at the new folder, restart `com.pillseek.reader`.
   Keep the old folder until the phone tests pass. Back up to `LLM_MODELS/pill-reader/`.

Lessons: v2.0 (2026-09-04, from Microsoft weights, outline shadows) scored 69.7% catalog / 32% phone-style
but misread real phone photos (`BX 1`, `X 1`) — rejected.

## Retraining v1 (Google Colab, A100, ~1 h, ~15–25 compute units)
...
LaunchAgents `com.pillseek.reader` (uvicorn :8002, `PILL_TROCR_DIR=pill_trocr_base`, `PILL_TROCR_DIR2=pill_trocr_large`,
