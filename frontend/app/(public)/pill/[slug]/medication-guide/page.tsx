...
export async function generateMetadata({
  ...
  return {
    ...
    description: `View the FDA medication guide for ${drugName}, including uses, dosage, side effects, and safety information.`,
...
export default async function MedicationGuidePage({
  ...
  if (!guideData?.has_medguide) notFound()
  ...
  const hasMedicationSummaryFallback = Boolean(
    guideData?.has_medication_summary || guideData?.medication_summary_html?.trim()
  )

  const drugSlug = slugifyDrugName(drugName)

  const mgRxcui = guideData?.rxcui ?? pill.rxcui
  const mgNdc = guideData?.ndc ?? pill.ndc11 ?? pill.ndc9
  const mgSplSetId = pill.spl_set_id
  const sanitizedMedguideHtml = guideData?.medguide_html
    ? sanitizeRenderedHtml(guideData.medguide_html)
    : null

  const mgPageJsonLd = guidePageSchema({
    drugName,
    slug,
  ...
  const mgBreadcrumbs = breadcrumbSchema([
    { name: 'Home', url: '/' },
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd(mgBreadcrumbs) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd(mgPageJsonLd) }} />
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        <DrugPageHeader
          pageLabel="FDA Medication Guide"
        ...
        <div className={SHARED_CONTENT_CARD_CLASSES}>
          {sanitizedMedguideHtml ? (
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 text-center">
              <p className="text-sm text-slate-800 mb-3">Medication guide is not available.</p>
        ...
        {(mgRxcui || mgNdc || guideData?.fetched_at || guideData?.source_url) && (
          <section className="border border-slate-200 rounded-xl p-4 text-xs text-slate-500 space-y-1">
            ...
            {mgRxcui && <p><span className="font-medium">RxCUI:</span> {mgRxcui}</p>}
            {mgNdc && <p><span className="font-medium">NDC:</span> {mgNdc}</p>}
