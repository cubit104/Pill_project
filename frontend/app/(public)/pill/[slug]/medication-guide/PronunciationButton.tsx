...
export default function PronunciationButton({ slug: _slug, drugName, speakerOnly, textOnly }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/pronunciation/${encodeURIComponent(drugName.toLowerCase())}/audio`)
      .then((r) => {
        if (!r.ok) {
          if (r.status !== 404) console.error(`Pronunciation fetch failed: ${r.status}`)
          return null
        }
        return r.json()
      })
  ...
  function handleClick() {
    if (data?.audio_url) {
      ...
      audio.onerror = () => { setPlaying(false); speakDrugName() }
  ...
  // speakerOnly mode — just the icon button, no text
  if (speakerOnly) {
    return (
      <button
        type="button"
