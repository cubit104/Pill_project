...
interface SearchBarProps {
  ...
  initialValues?: { q?: string; type?: string; color?: string; shape?: string }
}
  ...
  return (
     <div className="bg-emerald-50 rounded-2xl shadow-xl p-6 text-left border border-emerald-200">
      <div className="flex border-b border-slate-200 mb-4" role="tablist" aria-label="Search type">
        {TABS.map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1 ${
              activeTab === tab.id
      ...
      <div className="relative mb-4">
        <label htmlFor="search-input" className="sr-only">{TABS.find((t) => t.id === activeTab)?.label} search</label>
        <input
          id="search-input"
          ref={inputRef}
          type="text"
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
        <div className="flex-1">
          ...
          <select
            ...
            className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
        ...
        <div className="flex-1">
          ...
          >
            ...
            {shapes.map((s) => (<option key={s.name} value={s.name}>{s.name}</option>))}
          </select>
        </div>

        <button
          onClick={handleSubmit}
          className="bg-emerald-700 hover:bg-emerald-800 text-white font-semibold px-6 py-2.5 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 whitespace-nowrap inline-flex items-center justify-center gap-2"
          aria-label="Search for pills"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35m0 0A7.5 7.5 0 103.5 10.5a7.5 7.5 0 0013.15 6.15z"/></svg>
          Search
        </button>
      </div>
    </div>
