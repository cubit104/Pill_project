  ...
  return (
    <div className="border border-gray-300 rounded-md bg-white overflow-hidden medguide-editor">
      <div className="flex flex-wrap items-center gap-2 p-2 border-b border-gray-200 bg-gray-50">
        <button type="button" className={toolbarButton(!!editor?.isActive('bold'))} onClick={() => editor?.chain().focus().toggleBold().run()}>B</button>
        <button type="button" className={toolbarButton(!!editor?.isActive('italic'))} onClick={() => editor?.chain().focus().toggleItalic().run()}><em>I</em></button>
        <button type="button" className={toolbarButton(!!editor?.isActive('heading', { level: 2 }))} onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()}>H2</button>
        <button type="button" className={toolbarButton(!!editor?.isActive('heading', { level: 3 }))} onClick={() => editor?.chain().focus().toggleHeading({ level: 3 }).run()}>H3</button>
        <button type="button" className={toolbarButton(!!editor?.isActive('bulletList'))} onClick={() => editor?.chain().focus().toggleBulletList().run()}>•</button>
        <button type="button" className={toolbarButton(!!editor?.isActive('orderedList'))} onClick={() => editor?.chain().focus().toggleOrderedList().run()}>1.</button>
        <button type="button" className={toolbarButton(!!editor?.isActive('link') || showLinkInput)} onClick={openLinkEditor}>Link</button>
        <button type="button" className={toolbarButton(isSourceMode)} onClick={() => setIsSourceMode((prev) => !prev)}>{'</>'}</button>
      </div>
      {showLinkInput && (
      ...
      {isSourceMode ? (
        <textarea
          ...
          onChange={(e) => {
            ...
            onChange(nextHtml)
            if (editor && nextHtml !== lastSyncedContentRef.current) {
