...
def _build_public_pill_payload(conn, pill_info: Dict[str, Any], *, slug_fallback: str) -> Dict[str, Any]:
    raw_medicine_name = pill_info.get("medicine_name", "") or ""
    raw_splimprint = pill_info.get("splimprint", "") or ""
    raw_image_filename = pill_info.get("image_filename", "") or ""
    ...
    logger.info(
        "Slug %s: medicine_name=%r, splimprint=%r, found %s images, own_filename=%r",
        slug_fallback,
        raw_medicine_name,
        raw_splimprint,
        len(image_urls),
    ...
    try:
        ...
        if "pill_ndcs" in err_msg and (
            ...
            logger.warning("pill_ndcs lookup failed for %s: %s", slug_fallback, _e)
    ...
    try:
        ...
        if _is_missing_col:
            ...
            logger.warning("Medication guide flag lookup failed for %s: %s", slug_fallback, _e)
    ...
    mapped = {
        ...
        "brand_names_all": [],
        "is_brand_row": False,
        "is_draft": not bool(pill_info.get("published")),
    }
    ...
    history_resolution = _resolve_history_identifier(
        conn,
        slug=str(pill_info.get("slug") or slug_fallback),
    ...
    if rxcui_val:
        try:
            ...
            if ind_row:
                if hasattr(ind_row, "_mapping"):
                    row_map = ind_row._mapping
                else:
                    row_map = {
                        "plain_text": ind_row[0] if len(ind_row) > 0 else None,
                        "source_url": ind_row[1] if len(ind_row) > 1 else None,
                        "source": ind_row[2] if len(ind_row) > 2 else None,
                        "fetched_at": ind_row[3] if len(ind_row) > 3 else None,
                    }
                mapped["indication"] = {
...
def get_pill_by_slug(slug: str):
    ...
    try:
        with database.db_engine.connect() as conn:
            ...
            columns = result.keys()
            pill_info = dict(zip(columns, row))
            return _build_public_pill_payload(conn, pill_info, slug_fallback=slug)
...
@router.get("/api/pill/preview/{pill_id}")
def preview_pill(pill_id: str, admin: dict = Depends(get_admin_user)):
    """Preview a pill (draft or published) by ID — admin only."""
    if not database.db_engine:
        if not database.connect_to_database():
            raise HTTPException(status_code=500, detail="Database connection not available")
    ...
    try:
        with database.db_engine.connect() as conn:
            ...
            columns = result.keys()
            pill_info = dict(zip(columns, row))
            return _build_public_pill_payload(
                conn,
                pill_info,
                slug_fallback=str(pill_info.get("slug") or pill_id),
            )
    except HTTPException:
        raise
    except SQLAlchemyError as e:
        logger.error(f"preview_pill error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        logger.error(f"Error in preview_pill: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
