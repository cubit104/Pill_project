...
function SeverityBadge({ severity }: { severity: Severity }) {
  return <TextBadge tone={BADGE_TONE[severity]}>{SEVERITY_LABEL[severity]}</TextBadge>
}
    ...
    <Card tone={CARD_TONE[severity]} padded={false} className="overflow-hidden">
      ...
      >
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            ...
            {pair.found ? <SeverityBadge severity={severity} /> : <TextBadge tone="neutral">No data</TextBadge>}
          </span>
          <span className="mt-1 block text-[14px] leading-relaxed text-body">
            {pair.found ? pair.description ?? 'An interaction is recorded for this pair.' : pair.message ?? 'No known interaction was found for this pair.'}
          </span>
        </span>
      ...
      {open && detail && (
        <div className="border-t border-line px-4 py-3">
          <p className="section-label mb-1">What to do</p>
          <p className="selectable text-[14px] leading-relaxed text-body">{detail}</p>
          {pair.confidence && <p className="mt-2 text-[12px] text-muted">Confidence: {pair.confidence}</p>}
        </div>
      )}
    </Card>
  )
...
export default function InteractionsScreen() {
  ...
  const runCheck = useCallback(async () => {
    ...
    try {
      ...
      if (ctrl.signal.aborted) return
      setError(err instanceof ApiError ? err : new ApiError('unknown', 'Could not check interactions.'))
    } finally {
  ...
  return (
    <div ref={scrollRef} className="h-full overflow-y-auto bg-canvas animate-fade-up">
      ...
      >
        ...
        <p className="min-w-0 flex-1 truncate text-center text-[17px] font-semibold text-ink">Interactions</p>
      ...
      <main className="screen mx-auto max-w-lg space-y-4 px-4 pb-8 pt-2" style={{ paddingLeft: 'max(16px, var(--safe-left))', paddingRight: 'max(16px, var(--safe-right))' }}>
        <div className="px-1">
          <h1 className="text-[26px] font-bold leading-tight tracking-tight text-ink">Interaction checker</h1>
          <p className="mt-1 text-[15px] text-muted">Add two or more medicines to see how they affect each other.</p>
        </div>
        ...
        <section>
          ...
          <Card className="space-y-3">
            {drugs.length > 0 && (
              <ul className="flex flex-wrap gap-2" aria-label="Selected medicines">
                {drugs.map((d) => (
                  <li key={d}>
                    <span className="inline-flex h-10 items-center gap-1 rounded-full bg-brand-tint pl-3.5 pr-1 text-[15px] font-medium text-brand">
                      {d}
                      <button type="button" onClick={() => removeDrug(d)} aria-label={`Remove ${d}`} className="pressable flex h-8 w-8 items-center justify-center rounded-full">
            ...
            {drugs.length < MAX_DRUGS ? (
              <div className="relative">
                <TextField
                  ...
                  value={q}
                  onChange={setQ}
                  placeholder={drugs.length === 0 ? 'Add a medicine, e.g. Warfarin' : 'Add another medicine'}
                ...
                {suggestions.length > 0 && (
                  <ul className="card absolute inset-x-0 top-full z-10 mt-1 max-h-64 overflow-y-auto py-1" role="listbox" aria-label="Suggestions">
                    {suggestions.map((s) => (
                      <li key={s}>
              ...
              <p className="text-[13px] text-muted">Up to {MAX_DRUGS} medicines per check.</p>
            )}
            <Button full onClick={() => void runCheck()} disabled={!canCheck} loading={checking}>
              {drugs.length < 2 ? 'Add at least 2 medicines' : stale ? 'Check again' : `Check ${drugs.length} medicines`}
            </Button>
          </Card>
        </section>
        ...
        {result && !error && (
          <>
                ...
                <div className="grid grid-cols-4 divide-x divide-line text-center">
                  {(['major', 'moderate', 'minor', 'unknown'] as Severity[]).map((s) => (
                    <div key={s} className="px-1">
                      ...
                      <p className="text-[12px] text-muted">{SEVERITY_LABEL[s]}</p>
                    </div>
                  ))}
                </div>
                {stale && <p className="mt-2 text-center text-[12px] text-muted">List changed. Tap Check again to refresh.</p>}
              </Card>
            )}

            <section className={`space-y-3 ${stale ? 'opacity-60' : ''}`}>
              <SectionLabel>Drug to drug</SectionLabel>
              {sortedPairs.length === 0 ? (
                <Card padded={false}>
                  <EmptyState art="pill" title="No interactions found" body="Nothing recorded between these medicines. Still check with your pharmacist." />
                </Card>
              ) : (
            ...
            {result.disease_interactions.length > 0 && (
              <section className={`space-y-3 ${stale ? 'opacity-60' : ''}`}>
                <SectionLabel>Conditions to watch</SectionLabel>
                <Card padded={false} className="divide-y divide-line overflow-hidden">
        ...
        {!result && !checking && !error && drugs.length < 2 && (
          <Card padded={false}>
            <EmptyState art="search" title="Check your medicines together" body="Type a brand or generic name and pick it from the list. Add everything you take, including over-the-counter medicines." />
          </Card>
        )}
