...
export default function ContactScreen() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : t('Unable to send your message right now. Please try again later.'))
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div ref={scrollRef} className="h-full overflow-y-auto bg-canvas animate-fade-up">
      ...
      >
        ...
        <p className="min-w-0 flex-1 truncate text-center text-[17px] font-semibold text-ink">{t('Contact')}</p>
      ...
      <main className="screen mx-auto max-w-lg space-y-4 px-4 pb-8 pt-2" style={{ paddingLeft: 'max(16px, var(--safe-left))', paddingRight: 'max(16px, var(--safe-right))' }}>
        <div className="px-1">
          <h1 className="text-[26px] font-bold leading-tight tracking-tight text-ink">{t('Contact us')}</h1>
          <p className="mt-1 text-[15px] leading-relaxed text-muted">
            {t('Found a data error, have a question, or want to report a problem? We usually reply within 2–3 business days.')}
          </p>
        </div>
        ...
        {sent ? (
          <Card tone="tint" className="flex flex-col items-center py-8 text-center">
            ...
            <p className="mt-4 text-[18px] font-semibold text-ink">{t('Message sent')}</p>
          ...
          <Card className="space-y-4">
            ...
            <TextField label={t('Your name')} value={name} onChange={setName} placeholder={t('Your name')} autoComplete="name" autoCapitalize="words" enterKeyHint="next" />
            <TextField label={t('Your email')} value={email} onChange={setEmail} placeholder="your@email.com" type="email" inputMode="email" autoComplete="email" autoCapitalize="none" autoCorrect="off" enterKeyHint="next" />
            <textarea
              aria-label={t('Message')}
              value={body}
              onChange={(e) => setBody(e.target.value)}
              placeholder={subject === 'data-error' ? t('Which pill, and what is wrong? Include the imprint if you can.') : t('How can we help?')}
              rows={6}
              maxLength={4000}
        ...
        <Card padded={false} className="overflow-hidden">
          <button type="button" onClick={mailto} className="pressable flex w-full items-center gap-3 px-4 py-3.5 text-left active:bg-brand-tint">
            <span className="min-w-0 flex-1">
              <span className="block text-[16px] font-medium text-ink">{t('Or email us directly')}</span>
