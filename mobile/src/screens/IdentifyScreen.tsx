...
export default function IdentifyScreen({ active = true }: { active?: boolean }) {
  ...
  const runIdentify = useCallback(
    async (s1: CapturedPhoto, s2: CapturedPhoto) => {
      ...
      try {
        ...
        if (ctrl.signal.aborted) return
        const e = err instanceof ApiError ? err : new ApiError('unknown', err instanceof Error ? err.message : t('Unknown error'))
  ...
  const fallbackCapture = useCallback(
    async (source: 'camera' | 'photos', startSide: Side) => {
      try {
        ...
        toast.show(source === 'camera' ? t('Now flip the pill and take side 2') : t('Now choose the photo of side 2'))
        ...
        const msg = err instanceof CameraUnavailableError ? err.message : t('Could not open the camera')
  ...
  const submitFeedback = useCallback(
    async (verdict: 'up' | 'down', chosenSlug: string | null) => {
      ...
      try {
        ...
        toast.show(t('Thanks — your feedback helps the reader learn.'), 'success')
      } catch (err) {
        toast.show(err instanceof ApiError ? err.message : t("Couldn't save your feedback. Tap again to retry."), 'error')
      } finally {
        setFeedbackBusy(false)
      }
    },
    [result, feedback, feedbackBusy, imprintText, toast, t],
  )

  const runManual = useCallback(async () => {
    const tokens = tokenizeImprint(imprintText)
    if (!tokens.length && !color && !shape) {
      setManualError(new ApiError('bad_request', t('Type the imprint, or pick a colour or shape first.')))
      return
    }
    setManualBusy(true)
    setManualError(null)
    ...
    try {
      ...
      setManualError(err instanceof ApiError ? err : new ApiError('unknown', t('Something went wrong.')))
    } finally {
      setManualBusy(false)
    }
  }, [imprintText, color, shape, t])
      ...
      {([1, 2] as const).map((side) => {
        ...
        return (
          <button
            ...
            aria-label={photo ? t('Retake side {n}', { n: side }) : t('Take photo of side {n}', { n: side })}
          ...
          >
            {photo ? (
              <>
                <img src={photo.previewUrl} alt={t('Pill side {n}', { n: side })} className="h-full w-full object-cover" />
                <span className="absolute left-2 top-2 flex items-center gap-1 rounded-full bg-brand px-2 py-0.5 text-[12px] font-semibold text-brand-fg">
                  <CheckIcon size={12} strokeWidth={3} /> {t('Side {n}', { n: side })}
                </span>
              ...
              <>
                ...
                <span className="mt-3 text-[15px] font-semibold text-ink">{t('Side {n}', { n: side })}</span>
                <span className="mt-0.5 text-[13px] text-muted">{side === 1 ? t('Tap to capture') : t('Flip the pill')}</span>
              </>
            )}
          </button>
        )
      ...
      <div className="space-y-4">
        <Card>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="section-label">{t('Read from photo')}</p>
              <p className="selectable mt-1 text-[22px] font-bold tracking-tight text-ink">
                {result.imprint_read ? result.imprint_read : <span className="text-muted">{t('No imprint found')}</span>}
              </p>
              {result.attrs_guess?.shape && (
                <p className="mt-0.5 text-[14px] text-muted">
                  {result.attrs_guess.color
                    ? t('Looks {shape}, {color}', { shape: result.attrs_guess.shape.toLowerCase(), color: result.attrs_guess.color.toLowerCase() })
                    : t('Looks {shape}', { shape: result.attrs_guess.shape.toLowerCase() })}
                </p>
              )}
            </div>
        ...
        {matches.length === 0 ? (
          <Card padded={false}>
            <EmptyState
              art="pill"
              title={t('No confident matches')}
              body={t('Try a closer, sharper photo in good light, or type the imprint yourself.')}
              action={
          ...
          <>
            <SectionLabel>
              {matches.length === 1 ? t('1 match · compare with your pill') : t('{n} matches · compare with your pill', { n: matches.length })}
            </SectionLabel>
              ...
              {matches.map((m) => (
                <PillRow
                  ...
                  badge={
                    <ScoreBadge
                      value={m.similarity}
                      label={m.source === 'imprint' ? t('Imprint') : t('Visual')}
                  ...
                  footer={
                    result.capture_id && (!feedback || chosenSlug === m.slug) ? (
                      ...
                      >
                        <ThumbUpIcon size={18} />
                        {chosenSlug === m.slug ? t('Thanks — marked as your pill') : t('This is my pill')}
                      </button>
                    ) : null
                  }
                />
            ...
            {result.capture_id && (
              <div className="flex flex-wrap items-center justify-between gap-2 px-1">
                {feedback ? (
                  <p className="text-[14px] text-muted">{t('Thanks — your feedback helps us improve.')}</p>
                ) : (
                  <button
                    type="button"
                  ...
                  >
                    <ThumbDownIcon size={18} /> {t('None of these')}
                  </button>
                )}
  ...
  const renderManual = () => (
    <Card>
      <p className="section-label">{t('Imprint')}</p>
      <p className="mt-1 text-[14px] text-muted">{t('Letters and numbers on the pill, either side. Separate parts with a space.')}</p>
      <div className="mt-3">
        <TextField
          label={t('Imprint')}
          value={imprintText}
          onChange={setImprintText}
          placeholder={t('e.g. S 10')}
      ...
      {filters.colors.length > 0 && (
        <>
          <p className="section-label mt-4">{t('Colour')}</p>
          ...
          <p className="section-label mt-3">{t('Shape')}</p>
      ...
      {manualResult && (
        <div className="mt-4">
          {manualResult.candidates.length === 0 ? (
            <EmptyState art="search" title={t('No matches')} body={t('Try fewer imprint characters, or check the colour and shape.')} />
          ) : (
            <>
              <SectionLabel>{t('Possible matches')}</SectionLabel>
              <div className="card divide-y divide-line overflow-hidden">
                {manualResult.candidates.map((c) => (
                  <PillRow
                    ...
                    color={c.color}
                    shape={c.shape}
                    badge={<TextBadge tone={QUALITY_TONE[c.match_quality]}>{t(QUALITY_LABEL[c.match_quality])}</TextBadge>}
  ...
  if (featuresLoading) {
    ...
    body = (
      <ErrorCard
        error={new ApiError('offline', t("We couldn't reach PillSeek to check the photo reader. You can still search by imprint."))}
        onRetry={reloadFeatures}
        secondary={{ label: t('Go to Search'), onClick: () => navigate('/search') }}
      />
    )
  } else if (!enabled) {
    body = (
      <Card padded={false}>
        <EmptyState
          art="paused"
          title={t('Photo identification is paused')}
          body={t("We're tuning the pill reader right now. You can still search by imprint, drug name or NDC.")}
          action={
            <div className="flex gap-2">
        ...
        <ErrorCard
          ...
          secondary={
            error.kind === 'feature_off'
              ? { label: t('Go to Search'), onClick: () => navigate('/search') }
              : { label: t('Type the imprint instead'), onClick: () => setManualOpen(true) }
          }
        />
      ...
      <div className="space-y-4">
        <Card className="overflow-hidden">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 className="text-[20px] font-bold tracking-tight text-ink">{t('Identify a pill by photo')}</h2>
              <p className="mt-1 text-[15px] leading-relaxed text-muted">
                {t('Snap both sides in good light. Our reader decodes the imprint and matches it against 14,000 pills.')}
              </p>
            </div>
          </div>
            ...
            {bothSides ? (
              ...
              <>
                <Button full size="lg" icon={<CameraIcon size={22} />} onClick={() => openCamera(sides[1] ? 2 : 1)}>
                  {sides[1] ? t('Take side 2') : t('Open camera')}
                </Button>
        ...
        {cameraDenied && (
          <ErrorCard
            error={new ApiError('unknown', t('Camera access is off for PillSeek. Turn it on in your phone Settings → PillSeek → Camera, then try again. You can still choose photos from your library.'), { title: t('Camera access needed'), retryable: true })}
            onRetry={() => {
              setCameraDenied(false)
              openCamera(sides[1] ? 2 : 1)
            }}
            secondary={{ label: t('Choose photos'), onClick: () => void fallbackCapture('photos', sides[1] ? 2 : 1) }}
          />
        )}
        {renderSideTiles()}
        <p className="px-1 text-[13px] leading-snug text-muted">
          {consentLoaded && consent
            ? t('Photos you take are kept, without personal details, to improve the reader. Change this under About.')
            : t('Photos are analysed and not kept.')}
        </p>
        {sides[1] && !sides[2] && (
          <Card tone="tint" className="flex items-center gap-3">
            <FlipIcon size={22} className="flex-none text-brand" />
            <p className="text-[15px] text-body">
              <span className="font-semibold text-ink">{t('Now flip the pill')}</span> {t('and capture side 2 — we identify once we have both.')}
            </p>
          </Card>
        )}
        {(sides[1] || sides[2]) && (
        ...
        >
          {manualOpen ? t('Hide manual search') : t('Prefer to type the imprint?')}
        </button>
        {manualOpen && renderManual()}
        <Disclaimer compact />
      </div>
  ...
  return (
    <div ref={scrollRef} className="h-full overflow-y-auto">
      <ScreenHeader title={t('Identify')} subtitle={t('Photograph both sides of a pill')} scrollRef={scrollRef} />
      ...
      <Sheet open={noneSheet} onClose={() => setNoneSheet(false)} title={t('None of these?')}>
        <p className="text-[15px] leading-relaxed text-muted">
          {t("We'll log this so the reader improves. If the imprint we read was wrong, correct it first so we learn the right text.")}
        </p>
        <div className="mt-3">
          <TextField label={t('Corrected imprint')} value={imprintText} onChange={setImprintText} placeholder={t('Imprint on the pill')} autoCapitalize="characters" />
        </div>
        <div className="mt-4 space-y-2">
          <Button
            full
